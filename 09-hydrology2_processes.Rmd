...
::: {.example #ex-infilt1}
Fit a Horton infiltration model to measured infiltration data.
:::
...
\begin{equation}
  f_p~=~K_s\left[1~+~\frac{\Delta\theta~\Psi_f}{F}\right]
    (\#eq:GA1)
\end{equation} 
$\Delta\theta$ is the difference between effective porosity and initial water content and $\Psi$ is the capillary potential or wetting front suction head (L).
\begin{equation}
