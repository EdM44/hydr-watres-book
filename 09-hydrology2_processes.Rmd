...
Parameters for the Horton and Green-Ampt methods can be derived from observed infiltration data using the R package [HydroMe](https://CRAN.R-project.org/package=HydroMe). Fitting the Horton equation to observed data is demonstrated here using a small sample from the *infilt* data set included with the `HydroMe` package which is saved in a spreadsheet wrapped into the `hydromisc` package. While `HydroMe` includes a function to return parameters, this can also be done manually as shown here.

First, read in the data sample.
...
Set up a function for the Horton equation and use the built in `stats` package function nls to solve for the parameters.
...
infilt_data$Rate_estim <- predict(horton_fit, list(x = infilt_data$Time))
plot(infilt_data$Time,infilt_data$Rate, pch=1, 
     xlab = "Time, min", ylab = "Infiltration rate, cm/hr")
lines(infilt_data$Time, infilt_data$Rate_estim, lty = 1)
...
What is shown below is how to use [foreach](https://cran.r-project.org/package=foreach) in a brute force manner, running all simulations on a single CPU. 
...
Because the number of simulations quickly becomes very large, this command could be adapted to run the simulations in parallel using multiple cores by using the built-in R package [doParallel](https://cran.r-project.org/package=doParallel). By running the _foreach_ simulations with multiple processes, the run time using 7 of 8 available cores dropped from 5.9 to 1.1 minutes. Aside from setting up the cluster before the command and stopping it after (essential for freeing up resources), the only change is the use of _%dopar%_ instead of _%do%_.
