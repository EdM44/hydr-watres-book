...
```
The p-value is much lower than with the t-test, showing less influence of the two very high SWE values in the second half of the record. Output values can be extracted similarly to the t.test.
...
abline(m, lty=1, col="black")        # plots regression line
...
gevfit1$results$par
```
A plot of return periods (by specifying `type="rl"` for return level) for the fit distribution is available. 
