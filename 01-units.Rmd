...
df <- data.frame(
  ...
  US_Units = c("$ft$","$ft/s^2$","$slug$","$lb$","$slug/ft^3$","${ft}\\cdot{lb}$","$ft^{3}/s$=cfs","$ft^{2}/s$","$ft*lb/s$","$lb/in^2=psi$","$lb/ft^3$","$ft/s$","$lb*s/ft^2$","$ft^3$"),
  SI_units = c("$m$","$m/s^{2}$", "$kg$","$N$","$kg/m^3$", "${N}\\cdot{m}=joule (J)$", "$m^{3}/s$","$m^{2}/s$", "$N*m/s=watt (W)$", "$N/m^2=Pa$", "$N/m^3$","$m/s$","$N*s/m^2=Pa*s$","$m^3$"),
  Conversion = c(
    sig("ft","m"),
    sig("ft/s^2","m/s^2"),
    sig("slug","kg"),
...
One regular conversion that needs to occur is the translation between mass (m) and weight (W), where $W=mg$. Here $g$ is gravitational acceleration on the earth's surface: $g=9.81~m/s^2=32.2~ft/s^2$. When working with forces (such as with momentum problems or hydrostatic forces) be sure to work with weights/forces, not mass.
...
Repeating the unit conversion of viscosity with the _units_ package:
